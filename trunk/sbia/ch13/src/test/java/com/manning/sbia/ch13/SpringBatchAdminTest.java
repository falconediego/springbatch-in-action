...
public class SpringBatchAdminTest extends AbstractJobStructureTest {
	...
	private void startSpring() {
		context = new ClassPathXmlApplicationContext("/com/manning/sbia/ch13/SpringBatchAdminTest-context.xml");
	...
	private void startDatabase() throws Exception {
		File[] files = new File(".").listFiles(new FilenameFilter() {
			public boolean accept(File file, String filename) {
				return filename.startsWith("ch13.");
			}
		});
		for (File file : files) {
			file.delete();
